...
def register_query_config_and_measurement(query_path, disabled_rules, logical_plan, timed_result=None, initial_call=False) -> bool:
    ...
    if timed_result is not None and not storage.register_query_fingerprint(query_path, result_fingerprint):
        logger.warning('Result fingerprint=%s does not match existing fingerprints!', result_fingerprint)
...
def run_query_with_optimizer_configs(connector: connectors.connector.DBConnector, query_path):
    ...
    logger.info('Run Dynamic Programming for Query %s', query_path)
    ...
    logger.info('Found %s duplicated query plans!', num_duplicates)
